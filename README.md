...
* **Example:**
  ...
  const data = await padoNetworkClient.submitTaskAndGetResult(dataId, key.pk, key.sk, wallet)
  console.log(`data=${data}`);
  ```

#### getComputationPrice

Get the computing price of each node for each task. Now only supports AO’s test token wAR, minimum unit to use wAR(1 means 0.000000000001 wAR).
